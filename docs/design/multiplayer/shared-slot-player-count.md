...
- `ApRunData` saves the player's settings in native multiplayer contributions. Returning-player
  ...
  solo saves. Local checkpoint banks, server buff keys and durable check outboxes are separated
  by numbered player; new-run resets retain the connection's identity. Player 1 keeps its existing
  checkpoint paths and can read older metadata. Singleplayer checkpoints remain local.
...
### Integration with singleplayer checkpoints and bonus rewards

`ApSingleplayerSaves.CurrentIdentity` includes the selected player number. The checkpoint bank
rejects other owners while preserving legacy Player 1 paths and metadata. The save picker keeps
its higher canvas layer. The merge does not restore the old server checkpoint/recovery flow.

`SlayTheSpire2World.create_items` assigns each numbered player the configured bonus list and
subtracts it from that player's filler budget. Bonus receipt IDs retain their player prefix;
`BonusRewardUtility.ConvertToGold` removes that prefix before testing the universal item type.
Tests cover independent checkpoint owners, legacy metadata, bonus counts, budgets and seeded generation.
...
| One-player YAML | Existing IDs, character logic and Player 1 local checkpoint paths remain intact. |
